...
CLASSES = ['background', 'bicycle',  'car', 'motorbike', 'bus', 'bottle',
        'chair', 'diningtable', 'tv/monitor']
...
parser.add_argument("--data", type=str, default='/opt/data/private/diffkd_segmentation/dtp_transfer_lis',
                    help="The prefix for data directory")
parser.add_argument("--filelist", type=str, default='/opt/data/private/diffkd_segmentation/dataset/LIS_v2/test_dark.txt',
                    help="A text file containing the paths to masks")
parser.add_argument("--masks", type=str, default='/opt/data/private/diffkd_segmentation/dataset/LIS_v2/RGB-dark/test/masks',
...
def summarise_stats(M):
    ...
    pixelAcc=round(100.0*(acc1/acc2),3)
    mean.update_value(Metric.PixelAcc, pixelAcc)
    mean_bkg.update_value(Metric.PixelAcc, pixelAcc)
    ...
    sys.stdout.write("  pixelacc: {:.2f}\n".format(mean.summarize(Metric.PixelAcc)))
    print(split)
    ...
    sys.stdout.write("  pixelacc: {:.2f}\n".format(mean_bkg.summarize(Metric.PixelAcc)))
...
def evaluate_all(args):
    ...
    with open(args.filelist) as fd:
        ...
        for line in tqdm(fd.readlines()):
            ...
            filepath_gt = os.path.join(args.masks, files[0]+'.png')
            ...
            if with_oracle:
                ...
                basename=files[0]
                filepath = os.path.join(args.data, basename+'.png')
            ...
            image_name = files[0]#get_image_name(files[0])
