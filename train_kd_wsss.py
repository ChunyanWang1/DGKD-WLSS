...
from dataset.datasets import VOC12ClassificationDataset,VOC12ClassificationDatasetMSF
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='voc12',
                        help='dataset name')
    parser.add_argument('--data', type=str, default='/opt/data/private/diffkd_segmentation/dataset/VOC2012',
    ...
    parser.add_argument('--ignore-label', type=int, default=255, metavar='N',#-1
                        help='ignore label')
    ...
    parser.add_argument('--teacher-pretrained', type=str, default='./ckpts/model_enc_e020Xs0.928.pth',
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.kd_loss=DIST().to(self.device)
        if self.args.aux:
    ...
    def train(self):
        ...
        for iteration, (images, d_images,depth_images,targets,label, _) in enumerate(self.train_loader):
            ...
            with torch.no_grad():
                t_outputs = self.t_model(images,None,label)
                
            with autocast():
                s_outputs = self.s_model(d_images,depth_images,label)
                n,c=s_outputs[0].size()
                ...
                if iteration<=8000:
                    task_loss = F.multilabel_soft_margin_loss(s_outputs[0], label)
                else:
                    task_loss = F.multilabel_soft_margin_loss(s_outputs[0], label)+s_outputs[1].mean()
            ...
            kd_losses_f_reduced = self.reduce_mean_tensor(kd_loss_f)
            kd_losses_reduced_a = self.reduce_mean_tensor(kd_loss_f_aux)
            ddim_losses_reduced = self.reduce_mean_tensor(ddim_loss)
            ddim_losses_f_reduced = self.reduce_mean_tensor(ddim_loss_f)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    "Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || KD Loss: {:.4f}" \
                        " || DDIM Loss: {:.4f} || KDA Loss: {:.4f} || DDIMA Loss: {:.4f} || KDF Loss: {:.4f} || DDIMF Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        iteration, args.max_iterations, self.optimizer.param_groups[0]['lr'], task_losses_reduced.item(),
                        kd_losses_reduced.item(), ddim_losses_reduced.item(), kd_losses_reduced_a.item(), ddim_losses_reduced_a.item(), kd_losses_f_reduced.item(), ddim_losses_f_reduced.item(),
