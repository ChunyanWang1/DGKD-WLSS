...
def get_img_path2(img_name, voc12_root):
    if not isinstance(img_name, str):
        img_name = decode_int_filename(img_name)
    return os.path.join(voc12_root, IMG_FOLDER_NAME2, img_name + '.jpg')#'.jpg'

def get_img_path3(img_name, voc12_root):
    if not isinstance(img_name, str):
        img_name = decode_int_filename(img_name)
    return os.path.join(voc12_root, IMG_FOLDER_NAME3, img_name + '_depth.png') #_depth.png
...
class VOC12ImageDataset(data.Dataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
                 ...
                 crop_size=None, crop_method=None, to_torch=True,aug=False,rescale_range=[0.5, 2.0],img_fliplr=True):
    ...
    def __transforms(self, image,d_image,depth_image,mask):
        img_box = None
        local_image = None
        if self.aug:
            if self.rescale_range:
                #image = transforms.random_scaling(image, scale_range=self.rescale_range)
                image,d_image,depth_image,mask = transforms.random_scaling(image,d_image,depth_image,label=mask, scale_range=self.rescale_range)
            if self.img_fliplr:
                image,d_image,depth_image,mask = transforms.random_fliplr(image,d_image,depth_image,label=mask)
            if self.crop_size:
                image,d_image,depth_image,mask, img_box = transforms.random_crop(image,d_image,depth_image,label=mask, crop_size=self.crop_size, mean_rgb=[0,0,0], ignore_index=255)
            
            local_image = self.local_view(Image.fromarray(d_image))

        
        image = self.normalize(image)
        d_image = self.normalize(d_image)
        depth_image = self.normalize(depth_image)
        
        return image,d_image,depth_image,mask, local_image, img_box
    ...
    def __getitem__(self, idx):
        ...
        pil_image = Image.fromarray(d_img)
        img, d_img,depth_img,mask,local_image, img_box = self.__transforms(img,d_img,depth_img,mask)
        ...
        if self.aug:
            ...
            return {'name': name_str, 'img': img,'d_img':d_img,'depth_img':depth_img,'target':mask,'img_box':img_box}#,'img_box':img_box,'crops':crops
        else:
            return {'name': name_str, 'img': img,'d_img':d_img,'depth_img':depth_img,'target':mask}
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
                 resize_long=None, rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False,
                 crop_size=None, crop_method=None,aug=False,rescale_range=[0.5, 2.0]):
        super().__init__(img_name_list_path, voc12_root,
                 resize_long, rescale, img_normal, hor_flip,
                 crop_size, crop_method,aug)
        self.aug=aug
    ...
    def __getitem__(self, idx):
        ...
        if self.aug:
            return out['img'],out['d_img'],out['depth_img'],out['target'],out['label'],out['name'],out['img_box']#,out['crops'] #out['hist_img'] out['depth_img']
        else:
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, num_classes,img_normal=TorchvisionNormalize(), scales=(1.0,)):
        self.scales = scales

        #super().__init__(img_name_list_path, voc12_root, img_normal=img_normal)
        self.scales = scales
        self.voc12_root=voc12_root
        #self.transforms=T.Resize((512,512))#(448,448)(512,512)
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.img_normal=img_normal
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
