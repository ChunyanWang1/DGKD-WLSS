...
def encode_cmap2(label,img_ori):
    cmap = colormap()
    #cmap=colormap2()# for lis dataset
    color_map=cmap[label.astype(np.int16),:]

    # index = np.squeeze(label_img[:, :, 0])
    # #set ignored label to specific one
    # index[index==255] = 21
    # #label2rgb
    # index2rgb = cm[index]

    # index2bgr = index2rgb[:, :, [2, 1, 0]]

    alpha = 0.3
    color_map = alpha * img_ori + (1 - alpha) * color_map
    #index2bgr = alpha * img_ori + (1 - alpha) * gt_img
    #save
    #save_rgb_path = os.path.join(save_path, file_name+".png")
    #cv2.imwrite(save_rgb_path, index2bgr)
    return color_map
...
def colormap2(normalized=False):
    dtype = 'float32' if normalized else 'uint8'
    cmap = np.zeros((9, 3), dtype=dtype)
    #cmap=[]
    cmap[0]=np.array([0,0,0])
    cmap[1]=np.array([67, 159, 36])
    cmap[2]=np.array([53, 119, 181])
    cmap[3]=np.array([245, 128, 6])
    cmap[4]=np.array([127, 127, 127])
    cmap[5]=np.array([135, 86, 75])
    cmap[6]=np.array([204, 43, 41])
    cmap[7]=np.array([145, 104, 190])
    cmap[8]=np.array([219, 120, 195])
...
def tensorboard_image(imgs=None, cam=None,):
    ...
    # file_name=""
    # imageio.imsave(os.path.join('heatmap', file_name + '.png'), cam_img.type(torch.uint8))
...
def tensorboard_attn2(attns=None, size=[224,224], n_pixs=[0.0, 0.3, 0.6, 0.9], n_row=4, with_attn_pred=True):#[0.0, 0.3, 0.6, 0.9]
    n = len(attns)
    attns_top_layers = []
    attns_last_layer = []
    grid_attns = []
    ...
    #grid_attn_top_case4 = tensorboard_attn(attns_top_layers, n_pix=n_pixs[2], n_row=n_row)
    #grid_attn_top_case5 = tensorboard_attn(attns_top_layers, n_pix=n_pixs[3], n_row=n_row)
    ...
    #grid_attns.append(grid_attn_top_case4)
    #grid_attns.append(grid_attn_top_case5)
    ...
    #grid_attn_last_case4 = tensorboard_attn(attns_last_layer, n_pix=n_pixs[2], n_row=2*n_row)
    #grid_attn_last_case5 = tensorboard_attn(attns_last_layer, n_pix=n_pixs[3], n_row=2*n_row)
    ...
    #grid_attns.append(grid_attn_last_case4)
    #grid_attns.append(grid_attn_last_case5)
