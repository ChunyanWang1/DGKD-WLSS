...
from dataset.datasets_lis import LISClassificationDataset,LISClassificationDatasetMSF 
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='LIS',
                        help='dataset name')
    parser.add_argument('--data', type=str, default='/opt/data/private/diffkd_segmentation/dataset/LIS_v2', 
    ...
    parser.add_argument("--train_list", default="/opt/data/private/diffkd_segmentation/dataset/LIS_v2/voc_train_8_classes_train.txt", type=str)
    parser.add_argument("--infer_list", default="/opt/data/private/diffkd_segmentation/dataset/LIS_v2/voc_train_8_classes_val.txt", type=str)
    ...
    parser.add_argument('--aux', action='store_true', default=False,
                        help='Auxiliary loss')
    parser.add_argument('--batch-size', type=int, default=6, metavar='N',#16
                        help='input batch size for training (default: 8)')
    parser.add_argument('--start_epoch', type=int, default=0,
                        metavar='N', help='start epochs (default:0)')
    parser.add_argument('--max-iterations', type=int, default=40000, metavar='N',
                        help='number of epochs to train (default: 50)')
    parser.add_argument('--lr', type=float, default=0.005, metavar='LR',#0.02
    ...
    parser.add_argument('--save-dir', default='./work_dirs/dgkd-wlss-voc-lis/',
                        help='Directory for saving checkpoint models')
    parser.add_argument('--save-epoch', type=int, default=10,
                        help='save model every checkpoint-epoch')
    parser.add_argument('--log-dir', default='./work_dirs/dgkd-wlss-voc-lis/',
                        help='Directory for saving checkpoint models')
    ...
    parser.add_argument('--teacher-pretrained', type=str, default='./ckpts/model_voc_8_normal.pth',#'/opt/data/private/1-stage-wseg-master/snapshots/lis/baselines/v04/model_enc_e022Xs0.861.pth', #'/opt/data/private/diffkd_segmentation/ckpts/model_enc_e014Xs0.859.pth',#'/opt/data/private/diffkd_segmentation/ckpts/best_deeplabv3_resnet101_lis_os16.pth',#best_deeplabv3_resnet101_voc_os16.pth
                        help='pretrained seg model')
    parser.add_argument('--student-pretrained-base', type=str, default='./ckpts/ilsvrc-cls_rna-a1_cls1000_ep-0001.params',#resnet18-5c106cde.pth ilsvrc-cls_rna-a1_cls1000_ep-0001.params
                    help='pretrained backbone')
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_kd = DiffKD(train_dataset.num_class)#self.s_model, self.t_model,args.student_backbone,args.teacher_backbone,
        ...
        #self.s_model._criterion_kd_f2=self.criterion_kd_f2
        
        self.kd_loss=DIST().to(self.device)#CriterionKD()
        if self.args.aux:
            self.criterion_kd_aux = DiffKD(train_dataset.num_class)
    ...
    def train(self):
        ...
        self.s_model.train()
        scaler = GradScaler()
        for iteration, (images, d_images,hist_images,targets,label, _) in enumerate(self.train_loader):
            ...
            hist_images = hist_images.to(self.device)
            targets = targets.long().to(self.device)
           
            label=label.to(self.device)
            
            with torch.no_grad():
                t_outputs = self.t_model(images,None,label[:,1:])#,None
            with autocast():
                s_outputs = self.s_model(d_images,hist_images,label[:,1:]) #label[:,1:]
                ...
                if iteration<=8000:
                    task_loss = F.multilabel_soft_margin_loss(s_outputs[0], label[:,1:])
                else:
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    "Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || KD Loss: {:.4f}" \
                        " || DDIM Loss: {:.4f} || KDA Loss: {:.4f} || DDIMA Loss: {:.4f} || kd_fea Loss: {:.4f} || ddim_fea Loss: {:.4f}|| kd_fea_aux Loss: {:.4f} || ddim_fea_aux Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        iteration, args.max_iterations, self.optimizer.param_groups[0]['lr'], task_losses_reduced.item(),
                        kd_losses_reduced.item(), ddim_losses_reduced.item(), kd_losses_f_aux.item(), ddim_losses_f_aux.item(),  kd_losses_f.item(),ddim_losses_f.item(),kd_losses_f_aux.item(),ddim_losses_f_aux.item(), 
    ...
    def validation(self):
        ...
        for i, (images, d_images,depth_images,target,label, filename) in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                ...
                outputs = model(d_image,depth_image,None)
