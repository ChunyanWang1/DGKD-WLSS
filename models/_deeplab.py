...
class DeepLabHeadV3Plus(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, aspp_dilate=[12, 24, 36],aux=False,**kwargs):
    ...
    def forward(self, feature,d_feature):
        ...
        feature_diffusion=self.feat(feature['out'])

        if self.aux:
            auxout = self.auxlayer(feature['aux_out'])
            aux_feature_diffusion=self.feat2(feature['aux_out'])

        output_feature = F.interpolate(output_feature, size=low_level_feature.shape[2:], mode='bilinear', align_corners=False)
        return [self.classifier( torch.cat( [ low_level_feature, output_feature ], dim=1 ) ),auxout,feature_diffusion, aux_feature_diffusion]
