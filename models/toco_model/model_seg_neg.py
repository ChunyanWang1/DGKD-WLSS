...
class network(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=None, aux_layer=None,s_model=False):
        super().__init__()
        self.num_classes = num_classes
        self.init_momentum = init_momentum

        self.encoder = getattr(encoder, backbone)(pretrained=pretrained,s_model=s_model, aux_layer=aux_layer)
        ...
        self.feat1 = nn.Conv2d(self.in_channels[-1], 256, kernel_size=1, bias=False,)
        
        self.feat2 =nn.Conv2d(self.in_channels[-1], 256, kernel_size=1, bias=False,)
    ...
    def get_param_groups(self):
        ...
        param_groups[3].append(self.feat1.weight)
        param_groups[3].append(self.feat2.weight)
    ...
    def forward(self, x, d_x=None, cam_only=False, crops=None, n_iter=None):

        if d_x==None:
            cls_token, _x, x_aux = self.encoder.forward_features(x)
        else:
            cls_token,_x, x_aux = self.encoder.forward_features2(x,d_x)
        ...
        cam_aux = F.conv2d(_x_aux, self.aux_classifier.weight)#.detach()
        if cam_only:
            return cam_aux.detach(), cam.detach()
        ...
        if crops is None:
            #return cls_x4, seg, _x4, cls_aux,feat1,feat2
            return cls_x4, seg, cam,cam_aux, cls_aux,fea1,fea2
        else:
            return cls_x4, seg, cam,cam_aux, cls_aux, output_t, output_s,fea1,fea2