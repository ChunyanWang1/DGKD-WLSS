...
class DGF2TransformerBlock(nn.Module):
    def __init__(self, dim, condition_dim,norm_layer=nn.LayerNorm,mlp_ratio=4.):
        super().__init__()
        self.sft=Block(
                dim=dim, num_heads=12, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
                drop=0., attn_drop=0., drop_path=0, norm_layer=norm_layer, vis=False)
        # self.norm1 = norm_layer(dim)
        # self.attn = Attention(
        #     dim, num_heads=6, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0., vis=False)
        ...
        # SFT参数生成器
        self.sft_gamma = nn.Linear(condition_dim, dim)
        self.sft_beta = nn.Linear(condition_dim, dim)

    def feature_fusion(self,fea1,fea2):#,module
        att1=torch.sigmoid(fea1)
        att2= torch.sigmoid(fea2)
        guide_att=0.5*(1-att1)*(1-att2)+att1*att2 #0.5*
        new_fea=fea1+guide_att*(fea1+fea2)#guide_att*self.feat(torch.cat([fea1,fea2],dim=1)) #
        return new_fea

    def forward(self, x, condition,h,w):
        # x: [B, N, C], condition: [B, D]
        # x = self.attn(self.norm1(x)) + x
        # x = self.ffn(x) + x
        x,_=self.sft(x)
        # SFT调制
        gamma = self.sft_gamma(condition)#.unsqueeze(1)  # [B, 1, C]
        beta = self.sft_beta(condition)#.unsqueeze(1)    # [B, 1, C]
        new_x = gamma * x + beta
        ...
        fea1=self._reshape_BCHW(x[:,1:,:],h,w)
        fea2=self._reshape_BCHW(new_x[:,1:,:],h,w)
        new_fea=self.feature_fusion(fea1,fea2)
        n,c,_,_=new_fea.size()
        new_fea=new_fea.view(n,c,-1).permute(0,2,1)
        x=torch.cat([token1.unsqueeze(1),new_fea],dim=1)
...
class VisionTransformer_revise(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        #     self.depth_patch_embed=PatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
        ...
        self.condition_block= Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, norm_layer=norm_layer, vis=vis)

        self.dgf2_block=nn.ModuleList([
            DGF2TransformerBlock(dim=embed_dim, condition_dim=embed_dim) for i in range(3)
        ])
    ...
    def prepare_tokens(self, x,patch_x):
        B, nc, h, w = x.shape
    ...
    def forward_features(self, x):
        ...
        x = self.prepare_tokens(x,patch_x)
    ...
    def forward_features2(self, x,y):
        ...
        #patch_y=self.depth_patch_embed(y)
        patch_y=self.patch_embed(y)
        y=self.prepare_tokens(y,patch_y)

        x = self.pos_drop(x)
        y=self.pos_drop(y)
        condition,condition_weight=self.condition_block(y)
        #print(condition[0].size())
        embeds = []
        i=0
        for blk in self.blocks:
            x, weights = blk(x,)
            if i<=2:
                x= self.dgf2_block[i](x,condition,h//16,w//16)
            embeds.append(x)
            i=i+1
...
@register_model
def vit_base_patch16_224(pretrained=False,s_model=True, **kwargs):
    # if s_model:
    #     #print("s_model!!!!!!!!!!!!!!!!!!!")
    model=VisionTransformer_revise(
    ...
    model.pretrained_cfg=default_cfgs['vit_base_patch16_224']
    if pretrained:
        #print("load model!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # if s_model:
            # trained_state_dict = torch.load('/opt/data/private/diffkd_segmentation/ckpts/jx_vit_base_p16_224-80ecf9dd.pth', map_location="cpu")
            # #model_dict=model.state_dict()
            # model.load_state_dict(state_dict= trained_state_dict, strict=False)
        # else:
        load_pretrained(
            model, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3), filter_fn=_conv_filter,strict=False)#,strict=False
    return model
